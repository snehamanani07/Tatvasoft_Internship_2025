    ...
    public class MissionThemeController(IMissionThemeService missionThemeService) : ControllerBase
    {
        ...
        public async Task<IActionResult> DeleteMissionTheme(int id)
        {
            try
            {
                ...
                if (!res)
                    return NotFound(new ResponseResult() { Data = null, Result = ResponseStatus.Error, Message = "Theme not found or couldn't be deleted" });

                return Ok(new ResponseResult() { Data = res, Result = ResponseStatus.Success, Message = "Theme deleted successfully" });
            }
            catch (Exception ex)
            {
                return BadRequest(new ResponseResult() { Data = null, Result = ResponseStatus.Error, Message = "Error Deleting Theme" });
            }
        }
    }
}
