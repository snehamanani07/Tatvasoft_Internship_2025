...
namespace Mission.Repositories.Repositories
{
    public class LoginRepository(MissionDbContext missionDbContext): ILoginRepository
    {
        ...
        {
            var user = await _missionDbContext.Users.FirstOrDefaultAsync(u => u.Id == model.UserId);

            if (user == null)
                return (404, new { result = 0, message = "User not found." });

            if (user.Password != model.CurrentPassword)
                return (400, new { result = 0, message = "Current password is incorrect." });

            if (model.NewPassword != model.ConfirmPassword)
                return (400, new { result = 0, message = "New password and confirm password do not match." });

            user.Password = model.NewPassword;
            _missionDbContext.Users.Update(user);
            await _missionDbContext.SaveChangesAsync();

            return (200, new { result = 1, data = "Password changed successfully." });
        }
